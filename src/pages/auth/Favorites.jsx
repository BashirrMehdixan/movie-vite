...
import {FavouriteContext, MoviesContext, SeriesContext} from "/src/context/Context";
...
const Favorites = () => {
    ...
    const {likedMovie, likedSeries} = useContext(FavouriteContext);
    const favMovies = movies.filter(movie => likedMovie.includes(movie.id));
    const favSeries = series.filter(series => likedSeries.includes(series.id));
    return (
        <>
