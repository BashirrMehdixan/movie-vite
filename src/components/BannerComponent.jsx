...
export const BannerComponent = ({id, backdrop_path, original_title, name, overview, title, type}) => {
    ...
    const handleLike = async () => {
        ...
        const collectionName = location.pathname.includes("movies") ? "favoriteMovies" : "favoriteSeries";
        const docRef = doc(db, "users", user.id, collectionName, id.toString());
        console.log(docRef);
        const docSnap = await getDoc(docRef);
        if (docSnap.exists()) {
            await deleteDoc(docRef);
            dispatch({
                type: location.pathname.includes("movies") ? "TOGGLE_FAVOURITE_MOVIE" : "TOGGLE_FAVOURITE_SERIE",
                payload: {id}
            });
        } else {
            await setDoc(docRef, {
